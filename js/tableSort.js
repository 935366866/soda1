...
$(function(){
	...
	$("#submit_paras").click(function(){
		...
		var l=$('#samples>.part>.input_a_m>input');
		var formData =[];
			for(var i=0;i<l.length;i++){
				var value=$("#samples>.part>.input_a_m>input:eq("+i+")").val();
				formData.push(value);
			}
		...
		formData.push(radioS);
		formData.push(radioZ);
		var bo=$(".infoTwo tbody tr").length;
		for(var i=1;i<=bo;i++){
			formData.push($("[name=radio"+i+"]:checked").val());
		}
		var trLength=$(".infoTwo tbody tr input[type=text]").length;
		for(var i=0;i<trLength;i++){
		formData.push($(".infoTwo tbody tr input[type=text]:eq("+i+")").val());
		}
